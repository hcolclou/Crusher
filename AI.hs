...
-- AI Tests
aiTests = TestList [cTests, gTests, aiTestsLocal]

cTests = TestList [TestLabel "calculateOddsMult Test 1" comTestEmpty, -- TestLabel "calculateOddsMult Test 2" comTestSingle,
...
comTestEmpty = TestCase(assertEqual "for (calculateOddsMult _ _ [] _ _ _), " 0.0 (calculateOddsMult mempty [] [] W B 1))
-- comTestSingle = TestCase(assertEqual "for (calculateOddsMult mempty [W] [W] W B 1), " 1.0 (calculateOddsMult mempty [] [[W]] W B 1))
cosTestAI = TestCase(assertEqual "for (calculateOddsSingle mempty [] [[W]] W B 1), " 2.0 (calculateOddsSingle mempty [] [[W]] W B 1))
cosTestPl = TestCase(assertEqual "for (calculateOddsSingle mempty [] [[W]] B W 1), " (-2.0) (calculateOddsSingle mempty [] [[W]] B W 1))
cvTestWW = TestCase(assertEqual "for (calculateValue mempty [[W]] W), " 1.0 (calculateValue mempty [[W]] W))
cvTestWL = TestCase(assertEqual "for (calculateValue mempty [[W]] W), " 1.0 (calculateValue mempty [[B]] B))
cvTestBW = TestCase(assertEqual "for (calculateValue mempty [[W]] W), " 1.0 (calculateValue mempty [[B]] B))
cvTestBL = TestCase(assertEqual "for (calculateValue mempty [[W]] W), " 1.0 (calculateValue mempty [[W]] W))


gTests = TestList [TestLabel "generatePlayset Test 1" gpTestEmptyW, TestLabel "generatePlayset Test 2" gpTestEmptyB,
...
gpTestSingleW = TestCase(assertEqual "for (generatePlayset [] [[W, X]] W), " [] (generatePlayset [] [[W, X]] W))
gpTestSingleB = TestCase(assertEqual "for (generatePlayset [] [[B, X]] B), " [] (generatePlayset [] [[B, X]] B))

aiTestsLocal = TestList [{-TestLabel "choosePlay Test 1" cpTestWW, TestLabel "choosePlay Test 2" cpTestBW,
